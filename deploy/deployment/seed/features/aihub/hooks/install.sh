...
DEFAULT_IMAGE_TAG="inoriyuuki/aihub:0.3.0"
...
services:
  aihub:
    image: inoriyuuki/aihub:0.3.0
    container_name: sc-aihub-${NODE_ID}
    restart: unless-stopped
    ports:
      - "8080:8080"
    volumes:
      - ${DATA_DIR}:/data
      - ${CONFIG_DIR}/admin_password:/run/secrets/admin_password:ro
    env_file:
      - ${RENDERED_DIR}/.env
