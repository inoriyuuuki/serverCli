...
## 5.4 声明式运维控制台（控制面 UI + API）

旧的「迁移与运维控制台」（`/api/v1/migrate/*` + `/migrate` 页面）已移除，由
声明式运维 V2 取代（见 doc/14_MIGRATION_MATRIX.md 与新增模型文档）：

- **结构化 Operation V2**：`POST /api/v1/operations` 创建结构化 OperationRequest
  （顺序无关、未知字段拒绝），状态机含 planned/awaiting_approval/queued/dispatched/
  running/verifying/succeeded/failed/rolling_back/rolled_back/cancelled/result_unknown；
  CAS 并发迁移 + 幂等指纹（同 key 异 payload 冲突）；approve/cancel/status 端点。
- **声明式资源 API**（`/api/v1/clusters`、`/profiles`、`/declarative-nodes`、
  `/service-references`、`/release-cache`、`/backup-sets`、`/oss-syncs`、
  `/primary-transfers`；权限目录 `ops:read` / `ops:ops`）。
- **Agent 执行**：结构化任务经 Agent 写入 0600 `SERVERCLI_OPERATION_CONTEXT` +
  0700 `SERVERCLI_SECRET_DIR`，包装脚本调用 `servercli ops context <file>`
  （Go CLI 严格解析，不再用位置参数/正则）；argv 不含 Secret。
- **前端**：导航「运维操作」（`/operations`）、「集群」（`/clusters`）、
  「Release 缓存」（`/release-cache`）、「主节点迁移」（`/primary-transfers`）。
- 节点需部署 0.0.34+ 才会通告新版 `servercli-ops` 命令（Operation V2 结构化参数）。
