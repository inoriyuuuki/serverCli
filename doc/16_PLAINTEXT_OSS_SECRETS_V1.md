...
> ⚠️ **策略放宽（操作员决定，偏离原始安全条款）**：原始需求 V1 要求"Secret 内容不得通过
> API GET 返回前端、UI 只展示已配置/版本/hash、只允许覆盖、旧值不回显"。应操作员要求放宽为
> **可回显、可编辑**：新增 `GET /api/v1/deployments/secrets/{id}/value`（仅 admin 会话、响应
> `Cache-Control: no-store`、每次查看落审计 `deployment.secret.view` 且不含内容、不写日志），
> 前端 Secret 页可"查看/编辑"当前值。该放宽将明文 Secret 暴露给浏览器侧，属于**已知安全债务**，
> 建议生产环境谨慎使用（浏览器缓存/截图/插件风险），后续可改回只覆盖模式。
