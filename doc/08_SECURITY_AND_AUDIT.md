...
- 外部 AI 自助 API 使用 Access Token（`sct_*`）鉴权：库中仅存 SHA-256 哈希与前缀，明文只在创建时返回一次，日志/错误/示例不出现明文；过期/撤销后 API 立即拒绝并级联撤销关联 Lease、通知节点删除公钥。
- 每次可识别 Token 的请求写入 `api_token_usage_log`（路由、状态码、结果、来源；不含 Token 明文与敏感请求体）。
- Lease 到期 = `min(申请时长, Token 到期, 系统绝对上限)`；到期后即使调度器未更新，SSH 运行时校验（签名运行时 Token）也拒绝新连接。
- 使用专用低权限账号。
- 临时公钥按 Lease 安装，不复用。
...
- `admin` profile 默认关闭（Token 拥有申请 admin 的能力，但节点侧仍需 sudoers 且风险审计完整记录）。
...
- 申请（Access Token 自动审批）、拒绝、签发、续期、续期拒绝；
- 登录开始、操作、断开、到期、撤销和终止会话；
- Access Token 创建/撤销与每次使用日志；
- 全局/节点开关变化；
- 本地授权状态不一致。
