-- ServerCLI: 0010 — 部署节点串行约束（Deployment Node Serial）
-- 目的：保证同一节点在同一时刻最多只有一个 queued/running 的部署操作目标，
-- 使同一节点上的部署默认串行执行，避免并发部署互相覆盖（agent 状态竞争、
-- 文件/服务冲突）。
-- 实现：partial unique index，与 0009 中 uq_deployment_op_active_feature
...
-- 0009 的 deployment_operation_target(node_id) 普通索引可覆盖本查询的过滤。
